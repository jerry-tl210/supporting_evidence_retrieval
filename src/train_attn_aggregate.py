...
class SER_Trainer:
    ...
    def train(self, num_epochs, batch_size, acc_gradient, accumulation_steps=1, evaluate_train_set=False):
        ...
        for epoch_i in range(num_epochs):
            ...
            torch.save(self.model.state_dict(), self.trained_model_path / "model_epoch{0}_eval_em:{1:.3f}_precision:{2:.3f}_recall:{3:.3f}_f1:{4:.3f}_train_loss:{5:.3f}.m".format(
                epoch_i, dev_performance['sp_em'], dev_performance['sp_prec'], dev_performance['sp_recall'], dev_performance['sp_f1'], avg_loss))
            print('==============================================================\n')
