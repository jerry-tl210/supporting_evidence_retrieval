...
def test_train(lr, num_epochs, batch_size, model_file_name):
    ...
    train_data = json_load(config.HOTPOT_TRAIN)
    train_set = NextHopDataset(train_data[:20])
    logger.info("train_set has {} instances".format(len(train_set)))

    logger.info("Indexing dev_set")
    dev_data = json_load(config.HOTPOT_DEV)
    dev_set = NextHopDataset(dev_data[:10], is_eval=True)
    logger.info("dev_set has {} q_bertInp".format(len(dev_set)))

    trainer = HopExtractor_Trainer(train_set, dev_set, model, lr, model_file_name)
...
def train(lr, num_epochs, batch_size, model_file_name):
    ...
    logger.info("Indexing train_set ...")
    train_data = json_load(config.HOTPOT_TRAIN)
    train_set = NextHopDataset(train_data)
    logger.info("train_set has {} instances".format(len(train_set)))

    logger.info("Indexing dev_set")
    dev_data = json_load(config.HOTPOT_DEV)
    dev_set = NextHopDataset(dev_data, is_eval=True)
    logger.info("dev_set has {} q_bertInp".format(len(dev_set)))
...
class SER_Trainer:
    ...
    def train(self, num_epochs, batch_size):
        ...
        dataloader_train = DataLoader(self.train_set, batch_size=batch_size,
                                      shuffle=True, collate_fn=hop_baseline_collate,
        ...
        for epoch_i in range(num_epochs):
            ...
            for batch_i, batch in enumerate(tqdm(dataloader_train)):
                ...
                loss = self.model(batch['input_ids'],
                                  ...
                                  batch['hop_start_target'],
                                  batch['hop_end_target'],
                                  batch['ans_start_target'],
                                  batch['ans_end_target'],
                                  batch['span_type_target'])
                if self.n_gpu > 1:
                    loss = loss.mean()
            ...
            if epoch_i % self.eval_freq == 0:
                metrics = self.eval(batch_size * 5)
                print('epoch %d span_type_accuracy: %.3f span_em: %.3f' %
                      (epoch_i, metrics['span_type_accuracy'], metrics['span_em']))
                model_to_save = self.model.module if hasattr(self.model, 'module') else self.model
                torch.save(model_to_save.state_dict(),
                           str(self.trained_model_path /
                               "model_epoch{0}_span_type_accuracy:{1:.3f}_span_em:{2:.3f}".
                               format(epoch_i, metrics['span_type_accuracy'], metrics['span_em'])))
